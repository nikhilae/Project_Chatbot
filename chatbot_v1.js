...
app1.post("/dialogflow", express.json(), (req, res) => {
...
async function identify_user(agent)
{
  ...
  const snap = await client.db("mpdb").collection("phnuser").findOne({phno: phno});
...
function report_issue(agent)
{
  ...
  var dbo = db.db("mpdb");
	...
	var myobj = { name:u_name, issue:issue_val,status:status,time_of_creation :time_date,trouble_ticket:trouble_ticket };
